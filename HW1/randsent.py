...
class Grammar:
    ...
    def _load_rules_from_file(self, grammar_file):
        ...
        with open(grammar_file,"r") as f:
            for line in f:
                first_strip=line.split("#",1)[0]
                if not first_strip.strip():
                    continue
                sec_strip=first_strip.split( )
                weight=sec_strip[0]
                lhs=sec_strip[1]
                rhs=sec_strip[2:]
                if lhs not in self.rules:
                    self.rules[lhs]=[]
                self.rules[lhs].append((float(weight), rhs))
                # print(self.rules)
    ...
    def sample(self, derivation_tree, max_expansions, start_symbol):
        ...
        """
        # print(random.choices(self.rules[start_symbol][1],weights=self.rules[start_symbol][0]))
        
        cnt=0
        def dfs(start_symbol): 
            nonlocal cnt
            if start_symbol not in self.rules:
                return start_symbol
            if cnt>=max_expansions:
                return '...'
            cnt+=1
            w=[]
            sym=[]
            for weight,rhs in self.rules[start_symbol]:
                w.append(weight)
                sym.append(rhs)
                
            part=[]
            for i in random.choices(sym,weights=w)[0]:
                    part.append(dfs(i))
            ans=" ".join(part)
            if derivation_tree:
                return "( "+start_symbol+" "+ans+" )"
            return ans
        return dfs(start_symbol)
